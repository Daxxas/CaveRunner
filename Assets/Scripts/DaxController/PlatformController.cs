...
public class PlatformController : RaycastController
{
    ...
    Dictionary<Transform, Controller2D> passengerDictionnary = new Dictionary<Transform, Controller2D>();
    ...
    void MovePassengers(bool beforeMovePlatform)
    {
        ...
        {
            if (!passengerDictionnary.ContainsKey(passenger.transform))
            {
                passengerDictionnary.Add(passenger.transform, passenger.transform.GetComponent<Controller2D>());
            }
            if (passenger.moveBeforePlatform == beforeMovePlatform)
            {
                passengerDictionnary[passenger.transform].Move(passenger.velocity, passenger.standingOnPlatform);
            }
        }
    }
    ...
    void CalculatePassengerMovement(Vector3 velocity)
    {
        ...
        if (velocity.x != 0)
        {
            ...
            for (int i = 0; i < horizontalRayCount; i++)
            {
                ...
                if (hit && hit.distance != 0)
                {
                    ...
                    {
                        movedPassengers.Add(hit.transform);
                        float pushX = velocity.y - ((hit.distance - skinWidth) * directionX);
        ...
        //Passenger on top of a platform
